...
import {getClient} from "@/common/apolloCLient"
...
export const useStoreModelByType__Graphql: UseModelState = (model, key?: ModuleEnum | [ModuleEnum, string]) => {
  const _key = model.name ?? (!key ? '' : Array.isArray(key) ? dealNameSpace(key[0], key[1]) : key)
  const {actions, state} = model
  const [, setState] = useState(Object.create(null))
  if (!_key) {
  ...
  const query: GraphqlQuery = useCallback(async (query, params, option) => {
    setLoad(query, true)
    const res = await graphQLQuery(getClient())(query, params, option).catch(e => {
      setError(query, e)
    }).finally(() => {
      setLoad(query, false)
    })
  ...
  const mutate: GraphqlMutate = useCallback(async (mutation, params, option) => {
    setLoad(mutation, true)
    const res = await graphQLMutate(getClient())(mutation, params, option).catch(e => {
      setError(mutation, e)
    }).finally(() => {
      setLoad(mutation, false)
    })
